...
def excluded_names(panel: dict, as_of: pd.Timestamp) -> dict[str, set[str]]:
    """NSI Q5 and Quality Q5, computed once per calendar month and reused
    for every Friday in that month -- matches composite_v2's convention."""
...
def compute_composite(panel: dict, as_of: pd.Timestamp, weights: dict[str, float], ibs_earnings_conditioned: bool) -> pd.DataFrame:
    ...
    excl = excluded_names(panel, as_of)
