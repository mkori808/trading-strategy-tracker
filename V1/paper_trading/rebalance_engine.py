...
import signals  # noqa: E402 (V1/paper_trading/signals.py)
...
def build_universe_and_score(track_name: str, cfg: dict, as_of: pd.Timestamp) -> dict:
    track_cfg = cfg["tracks"][track_name]
    panel = signals.load_live_panel(cfg["sharadar_db"], as_of, cfg["universe"]["exclude_fama_sectors"])
    result = signals.compute_composite(panel, as_of, track_cfg["weights"], track_cfg.get("ibs_earnings_conditioned", False))
...
def load_current_holdings_csv(ledger_dir: Path, track_name: str) -> set[str]:
    """Reads the most recent ledger row's holdings, if any tracking column
    exists; ledger.csv here only stores summary stats (per spec), so the
    actual current-holdings snapshot for turnover comparison is read from
    a sibling holdings_<track>.json this engine also maintains."""
    path = ledger_dir / f"holdings_{track_name}.json"
    if not path.exists():
        return set()
    return set(json.loads(path.read_text()))


def save_current_holdings(ledger_dir: Path, track_name: str, holdings: set[str]) -> None:
    path = ledger_dir / f"holdings_{track_name}.json"
    path.write_text(json.dumps(sorted(holdings)))
...
def dry_run_report(track_name: str, cfg: dict, as_of: pd.Timestamp) -> dict:
    result = build_universe_and_score(track_name, cfg, as_of)
    held = construct_portfolio(result, cfg["portfolio"]["notional_usd"])
    ledger_dir = HERE / "ledger"
    prior_holdings = load_current_holdings_csv(ledger_dir, track_name)
