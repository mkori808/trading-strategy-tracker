...
async def _start_execution_scheduler() -> None:
    ...
    async def _loop() -> None:
        while True:
            ...
            # RETIRED 2026-09-06 (see LESSONS.md): the DM/MRM blend shadows
            # (fixed 50/50, vol-scaled), the regime ledger, and the
            # hourly/plain-vs-residual shadows all fed the "does Dual
            # Momentum have an edge" research line, which closed 2026-08-11
            # ("Dual Momentum is momentum, not an edge"). Canonical Dual
            # Momentum's real paper-execution loop above and its dedicated
            # forward test (engine/forward_tracking.py, unaffected by this
            # closure) are unrelated infrastructure and keep running. These
            # four modules stop advancing here; each ledger carries its own
            # retirement marker and none of their historical observations
            # were deleted.
...
# RETIRED 2026-09-06 (see LESSONS.md): `_start_prop_snapshot_collector`
# advanced `engine.prop_forward`'s prop-scaling and self-funded shadows,
# both surplus forward shadows from the Dual Momentum research line that
# closed 2026-08-11. The five-minute Alpaca-equity-sampling loop no longer
# runs; `prop_forward.db`'s historical observations were not deleted and
# `engine.prop_forward.status()` still reads them, just with no new rows
# arriving. See engine/prop_forward.py's module docstring for the closure
# note.
