...
## 2026-09-05 update — Sharadar Bundle (Full History) purchased; supersedes the "do not buy yet" call below

The user purchased Sharadar's paid Bundle/Full History entitlement between the
2026-09-05 free-tier evaluation (`LESSONS.md`, `reports/sharadar_contract_validation/`)
and this update. The upgrade was **verified empirically, not taken on the
user's word**: `engine.validate_sharadar_contract.detect_tier(force_refresh=True)`
confirmed `full_history_or_broader` (pre-2021 WMT history and a genuinely
non-Dow symbol, TSLA, both now return rows). This does not retract the
CRSP/Norgate analysis below — it was correct given what was available on
2026-08-22 — but Sharadar was not evaluated in that pass and its paid tier
closes several of the gaps that motivated recommending CRSP/Norgate. This
section is the capability matrix Phase 2 of the current session's task
required, measured against `engine/pit_all_stocks.py`'s exact contract
constants (never a re-typed second list).

### Capability matrix (measured against the paid `full_history_or_broader` entitlement)

| Domain | Source/table | Available? | Coverage | PIT notes |
|---|---|---|---|---|
| Security master | `tickers` (`permaticker`, `isdelisted`, `category`, `exchange`, `firstpricedate`/`lastpricedate`) | Partial | All 29/29 measured `EQUITY_UNIVERSE` symbols; `isdelisted='Y'` correctly set on confirmed-delisted names (SWY, LO, COV empirically checked) | `permaticker` is documented permanent but **identity continuity is unverified**: 0 ticker-change action rows found across the 29-symbol Dow roster (too small a sample to contain a real ticker-rename event — this is an untested-population gap, not a failed test). `exchange`/`category` are current-snapshot only, no point-in-time security-type intervals. |
| Daily prices | `stocks` (`open/high/low/close/closeadj/closeunadj/volume`) | Yes | Full history (pre-2021 WMT confirmed); daily, all 4 corporate-action-adjustment tiers documented and empirically verified (WMT 3-for-1 split test) | **Never trust vendor `closeadj` directly** — see reconstruction findings below. `RawClose`=`closeunadj` is PRESENT and genuinely raw (never split-adjusted). Adjusted `Open/High/Low` must be imputed by ratio (`stocks.{o,h,l} * closeadj/close`); Sharadar delivers no adjusted OHL directly. |
| Delisted securities | `tickers.isdelisted` + `stocks` (price history continues through delisting) | **Yes — the free-tier wall is resolved** | Empirically confirmed on 3 real 2015 S&P 500 M&A delistings (SWY, LO, COV): full daily price history present through the actual `lastpricedate` (matches deal-close date) | This is the single biggest capability upgrade over every prior free-data effort in this repo — `data/sp500_pit_free/`'s own coverage audit found yfinance could not serve 46% of historical S&P tickers at all (worst-case window 39% coverage), specifically because it cannot fetch delisted names. Sharadar's paid tier serves exactly the population that effort failed on. |
| Corporate actions | `actions` (`split`, `dividend`, `spinoffdividend`, ticker-change, others) | Yes | Dated, typed events; `value` semantics empirically decoded (see reconstruction module docstring) | No documented per-action-type `value` semantics from the vendor — decoded empirically instead (split=share ratio; dividend/spinoffdividend=dollar amount against the SPLIT-ADJUSTED prior price, not raw price). |
| Fundamentals | `daily` (`marketcap`, `pe`, `pb`, `ps`, `ev`, ...) | Yes (snapshot-shaped) | Daily granularity, so genuinely point-in-time by DATE — unlike `engine/fundamentals.py`'s `yfinance.Ticker.info` snapshot tier | Not yet contract-mapped beyond `MarketCap` (`MARKET_CAP_COLUMN`, confirmed PRESENT). Full fundamentals (`fundamentals`/`metrics` tables) were not probed this session — out of scope for the index-deletion hypothesis, which needs prices and membership only. |
| Filing/publication dates | `events` (SEC Form 8-K `eventcodes`, ticker+date primary key) | Untested this session | Documented to cover "each active and delisted ticker" | Not empirically probed; `eventcodes` taxonomy (whether it distinguishes delisting reasons) is unconfirmed. Added to `SharadarClient.ALLOWED_TABLES` for future use but not queried. |
| Historical S&P 500 membership | **`sp500`** (`date`, `action` = added/removed/current/historical, `ticker`, `name`, `contraticker`, `contraname`, `note`) | **Yes — a major, previously-unknown capability** | Empirically pulled full history: 59,092 rows 1998-01-01 to 2026-09-05; 696 `removed` + 699 `added` discrete events; 503 `current` + 57,194 `historical` (quarterly) snapshot rows | **Resolves the exact gap the Phase-1 infrastructure audit flagged as MISSING**: `note` gives free-text removal reasons on every single removal event (100% populated, 0 blank), machine-classifiable by keyword into acquired/merged (370), market-cap-change (184), spinoff (73), bankruptcy (10), other (59) — i.e. Sharadar itself distinguishes M&A-driven removals from float/cap-driven ones, which `data/universe_membership.json`'s free community ledger cannot do. Not previously known to exist in this repo; discovered by reading Sharadar's own docs index (`sharadar.com/docs/intro`), not assumed from the 4-table free-tier audit's scope. |
| Other index membership (S&P 400/600) | none | **No** | — | Empirically probed and confirmed absent: `sp400`, `sp600`, `midcap400`, `smallcap600`, `indices` all return HTTP 403 under the current entitlement. Sharadar's own docs index lists only "S&P500 Constituents," no midcap/smallcap equivalent. This is a genuine, pre-existing data-availability constraint, decided before any hypothesis screen ran — not a scope narrowed after a disappointing result (see `research/hypothesis_backlog.md`'s index-deletion entry). |
| Sector/industry metadata | `tickers` (`sicsector`, `sicindustry`, `famaindustry`, `sector`, `industry`) | Yes (current-snapshot) | Observed directly on SWY/LO/COV pulls | Same current-only caveat as `exchange`/`category` above — no point-in-time sector reassignment history. |

### Vendor `closeadj` reconstruction findings (generalized from the 2026-09-05 entry below)

`engine/reconstruct_adjustment.py` (generalized from a HON-only diagnostic)
independently recomputes total-return close from `closeunadj` + `actions`,
never referencing vendor `closeadj`, and cross-checks against it. Run
against the 5 symbols the Dow-close cross-check flagged:

| Symbol | Reconstruction vs. vendor `closeadj` | Verdict |
|---|---:|---|
| HON | 3.25% median divergence | **Vendor closeadj has a real defect**, isolated to one compound 2026-06-29 split+spinoff event |
| DOW | 0.10% median, 1.15% max (one event) | Vendor closeadj lags the most recent (5-day-old) ex-dividend date — a transient recency artifact, not a systemic defect |
| NKE | 0.013% median | Vendor closeadj is correct; the original discrepancy was the local yfinance cache carrying a different adjustment vintage |
| HD | 0.032% median | Same as NKE |
| MCD | 0.006% median | Same as NKE |

**Decision for the PIT store's price basis**: use the `closeunadj` + `actions`
reconstruction as the canonical adjusted close, exactly as CLAUDE.md's
Sharadar section specifies, with vendor `closeadj` retained only as an
independent cross-check (`engine.reconstruct_adjustment.run_panel` is the
permanent detector for this defect class across the full panel, not a
one-off HON fix).

### What this changes about the CRSP/Norgate recommendation below

The original 2026-08-22 analysis rejected a comprehensive all-stocks feed
because no reviewed source (Norgate, EODHD, Massive, the free community
ledger) could satisfy delisting-return/final-value and historical
index-membership requirements simultaneously. Sharadar's paid tier now
supplies: full price history through delisting (closing the free-tier's
worst gap), dated corporate actions sufficient for a from-scratch
adjustment reconstruction, and a native historical S&P 500 membership table
with machine-readable removal reasons. **It still does not supply an
explicit `DelistingReturn` field** (confirmed MISSING, not merely untested,
under the paid tier) — a terminal return for e.g. a bankruptcy delisting
must still be derived (last traded `closeunadj` vs. zero, or from `actions`
if a cash/stock consideration is recorded) rather than read directly. That
remaining gap is smaller and more tractable than the identity/coverage gaps
CRSP was recommended to close, but it is not yet closed, and the Phase 6
delisting-return acceptance gate this session's task specifies has not been
built or run — the PIT store implementation described below (Phase 3-7)
was not undertaken this session because the first hypothesis it would have
served screened out at the design stage before any store was needed (see
`research/hypothesis_backlog.md`).
