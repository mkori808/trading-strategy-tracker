...
- `sharadar_pit_adapter.py` — diagnostic-only normalization of Sharadar `tickers`/`actions` rows; derives available identity/action fields but fails closed on missing `known_at` and terminal delisting returns.
- `sharadar_pit_policy.py` — explicit supplemental-evidence policy for externally verified `known_at` timestamps and sourced terminal delisting values; no action date is silently treated as public-information time.
- `validate_pit_supplements.py` — validates the optional `data/pit_us_all_stocks/supplemental_evidence.json` file; missing, duplicate, unsourced, or malformed evidence fails closed.
- `build_pit_supplement_template.py` — creates a blank ten-case template from the existing reorganization ground-truth set; blank rows are intentionally non-publishable.
- `pit_acceptance_report.py` — assembles the current PASS/PARTIAL/FAIL matrix and ten-case evidence status; it emits `PIT_BLOCKED` until all critical normalized fields are evidenced.
- `build_pit_evidence_worklist.py` — derives the field-specific `known_at` and terminal-value sourcing worklist from the existing ten-case template.
- `data_capabilities.py` — capability registry and hypothesis-specific data-readiness gate; a limitation blocks only hypotheses that critically depend on it.
...
### Pre-research edge triage

The evidence-only triage funnel screens mechanism ideas before any strategy
code, returns, holdout, or preregistration is touched. It applies observability,
independent-breadth/MDA, cost, mechanism, and human-review gates in that order:

```powershell
python -m engine.edge_candidate_triage
python -m engine.ex_dividend_feasibility_screen
python -m engine.source_breadth_audit
```

`edge_candidate_triage.py` combines the backlog, breadth, cost, and identity
evidence into a ranked report. `ex_dividend_feasibility_screen.py` uses a
mechanism-supported five-session horizon and short Alpaca IEX quote windows;
one-venue quotes are sanity-checked against mega-cap names and are never
treated as consolidated NBBO. `source_breadth_audit.py` evaluates whether free
S&P 400/600 membership histories are long and complete enough to justify
infrastructure. The sponsor findings for leveraged ETFs are recorded in
`reports/leveraged_etf_source_audit/`.

These tools are diagnostic-only and fail closed. In the current evidence they
produce no `STRONG_CANDIDATE`: ex-dividend pressure is `COST_SENSITIVE`, the
S&P-family source audit is `STOP_SCREENED_OUT_BREADTH`, and leveraged-ETF reset
flow remains `DATA_BLOCKED_PARTIAL_UNLOCK` pending historical derivative
exposure and independent sponsor coverage.
