...
## 2026-09-06 — Sharadar is now gated by capability, not by a universal PIT binary

The evidence now supports a capability registry rather than treating the
entire provider as either `PIT_READY` or `PIT_BLOCKED`. Sharadar is
`SUPPORTED` for daily prices, unadjusted prices, basic corporate actions,
daily market cap, PIT filing-date fundamentals, and historical S&P 500
membership. Delisted history, adjusted prices, ticker history, permanent
identity, and terminal delisting economics are usable only with explicit
limitations and fail-closed policies. S&P 400/600 membership and complete
successor-security/reorganization linkage remain unsupported.

The non-negotiable adjusted-price carve-out survives the policy change:
`engine/reconstruct_adjustment.py` is canonical from `closeunadj` plus
`actions`; vendor `closeadj` is an independent cross-check because of HON's
3.25% compound-action defect. This is an intentional research-policy choice,
not a claim that every vendor field must be independently rebuilt.

`engine/data_capabilities.py` centralizes capability statuses, limitation
policies, hypothesis requirements, and fixed materiality thresholds (>5% of
security-periods or >10% of estimated trades). The readiness report shows
S&P 500 deletion and microcap momentum as `DATA_READY`, ordinary delisted
research with explicit episode exclusions as `DATA_READY`, ticker-change
momentum as `DATA_READY_WITH_LIMITATIONS`, and distressed/post-reorg plus
S&P 400/600 studies as `DATA_BLOCKED`. No returns, MDA, holdout, strategy, or
universe enablement was touched.

Evidence: `reports/data_readiness/report.md`.

---

## 2026-09-06 — Cheap follow-up checks closed the current triage loop without promoting a candidate

The ex-dividend harvestability follow-up used a mechanism-supported five-session
horizon. Corwin-Schultz implied 11.22% annualized drag versus Abdi-Ranaldo below
resolution; direct Alpaca IEX top-of-book quotes had a 364.39 bps median across
the liquid sample and failed a mega-cap sanity gate because MSFT measured 6.80
bps (threshold 5 bps). IEX is one venue, so its wide books cannot be treated as
a consolidated execution-cost estimate. The candidate remains
`COST_SENSITIVE`; no returns or holdout were inspected.

The bounded S&P-family source audit found pitindex coverage of the S&P 600 only
from 2021-03-26 and indexkit coverage from approximately 2019-11. Under a
transparent three-family breadth projection, at least 10.26 years of complete
history are needed to bring the existing 4.16% MDA proxy under the 4% ceiling.
Official S&P press releases verify individual changes but were not an exhaustive
machine-bounded ledger. Verdict: `STOP_SCREENED_OUT_BREADTH`.

The leveraged-ETF source check corrected one data assumption: ProShares publishes
historical daily NAV, shares outstanding, and AUM in its historical NAV CSV.
That removes one blocker for a bounded sponsor study, but derivative holdings/
notional exposure and an independent sponsor cross-check remain unverified. The
candidate stays `DATA_BLOCKED` and no flow model is built.

Evidence: `reports/ex_dividend_feasibility/`,
`reports/source_breadth_audit/`, and the regenerated
`reports/edge_candidate_triage/`.

---

## 2026-09-06 — Pre-research triage found no strong candidate; nominal rows again overstated breadth, and liquid mechanisms moved the bottleneck to observability or estimator disagreement

`engine/edge_candidate_triage.py` applied the existing observability,
`screen_design`, corrected dual-estimator cost, identity, and research-family
rules to the full open backlog plus seven source-supported mechanisms. It did
not inspect candidate returns, create a preregistration, or consume a holdout.
The result is deliberately negative: **zero candidates reached
`STRONG_CANDIDATE`**.

Three rules are now standing policy rather than post-hoc lessons:

1. **Gate order is observability → independent breadth/MDA → cost → mechanism
   → human review.** A PARTIAL population fails closed even when the mechanism
   is Tier 1. Mechanism quality is a prior and cannot rescue an upstream data,
   breadth, or cost failure.
2. **Rows are not bets.** The S&P 500 equal-weight reset produces roughly
   2,000 constituent/rebalance rows each year, but they are generated by only
   four common quarterly rule events. Counting four independent batches gives
   an MDA near 17.36%/yr, so the idea dies on breadth before AUM, flow, or
   return engineering. The annual Fed stress-test family has the same shape:
   many bank rows but one common supervisory/macro batch, yielding an MDA near
   40%/yr.
3. **Exact zero from a spread estimator means below estimator resolution, not
   free execution.** Ex-dividend pressure was the only new, observable design
   to clear the 4% breadth ceiling after 6,743 nominal 2025 dividend rows were
   collapsed to 248 date clusters (10-year MDA 3.12%). It remains
   `COST_SENSITIVE`: a disclosed 20-session holding assumption implies 2.80%
   annual drag from Corwin-Schultz while Abdi-Ranaldo reports zero. That
   disagreement, plus value/dividend/microstructure confounding, blocks
   promotion.

The highest-value next actions are small data audits, not strategies:
(a) look for one verifiable PIT S&P 400/600 membership-change source, because
the liquid 500-only migration proxy was already marginal at 4.16% MDA; and
(b) test whether 3–5 leveraged-ETF sponsors expose stable historical daily
NAV/share files sufficient to size their mandatory daily reset. Stop if either
dependency is not bounded. IPO lockups, Reg SHO Rule 201, 13F fire sales, and
post-reorg equity remain data-blocked for the exact trigger/population required;
their narratives do not justify infrastructure by themselves.

Full ranked evidence: `reports/edge_candidate_triage/report.md` and `.json`.

---

## 2026-09-06 — Distressed/post-reorg equity's data-shape caveat resolved: CIK matching verifies a link but cannot discover one, so the population is DATA_BLOCKED at scale

Direct follow-up to the cost-screen correction below, on the one remaining
open caveat: does Sharadar's data let a pre-bankruptcy security be linked
to its post-reorganization equity using only point-in-time information?
`engine/reorg_identity_screen.py`, tested against 10 real, independently-
verifiable bankruptcy cases (Hertz, Frontier, Chesapeake, GM, Washington
Mutual, Sears, SVB Financial, Party City, Rite Aid, Bed Bath & Beyond) --
picked for real-world diversity (successful reorg, pure liquidation,
acquisition-in-bankruptcy, ticker reuse by an unrelated company, repeated
restructuring), not for convenience.

**The false-link mechanism, finally explained rather than just observed.**
Last session's cost screen used `bankruptcyliquidation`-tagged tickers
without knowing whether any of them had a real successor. This session
found WHY `relatedtickers` looked misleading on Washington Mutual's WAMUQ
(pointing to "WM", now Waste Management Inc): the `tickerchangefrom`/
`tickerchangeto` pair immediately preceding a bankruptcy delisting is
Sharadar's OWN internal relabeling of the pre-bankruptcy ticker to a
bankruptcy-suffixed one (WM -> WAMUQ) -- a bookkeeping artifact, not a
claim about corporate succession. The same field pattern also MISSED a
real, correct link in the opposite direction: Frontier's FTRCQ never
mentions its actual 2021 successor FYBR anywhere in its own action or
relatedtickers history. Unreliable in both directions, confirmed on real
data rather than assumed from the schema.

**What actually works: the SEC CIK embedded in `tickers.secfilings`, an
externally-audited identifier Sharadar itself does not construct.**
Verified on three real reorganizations (Hertz, Frontier, Chesapeake) that
the pre- and post-bankruptcy securities share the SAME CIK -- the legal
registrant persisted through Chapter 11 even though Sharadar correctly
assigns the post-reorg security a DIFFERENT permaticker (the security, not
the issuer, is what's discontinuous: old shares cancelled, new ones
issued). Verified on GM (a genuinely new Delaware corporation formed after
"Motors Liquidation Company" wound down) and Washington Mutual (an
unrelated company) that a DIFFERENT CIK correctly flags non-continuity.
10/10 ground-truth cases classified correctly by this method alone.

**The decisive limitation: CIK matching VERIFIES a candidate; it does not
DISCOVER one.** Every one of the 10 confirmed cases required already
knowing which ticker to check externally (news, public bankruptcy
records) before the CIK match could confirm or reject it. No field in
`tickers` or `actions` points forward from a bankrupt security to its
successor, and none points backward from a new listing to a bankrupt
predecessor. A blind, market-wide discovery process would require
building a full CIK index across every Sharadar-tracked security and
matching candidates by (CIK, date proximity) -- real, substantial
infrastructure, deliberately not built in this pass per the explicit
instruction to validate on a small ground-truth sample before touching
the whole historical universe.

**Verdict: DATA_BLOCKED, not UNDERPOWERED or COST_BLOCKED.** The
mechanism is sound (verification works, 10/10) but not yet discoverable
at scale with existing fields, so the previous 2.82%/yr MDA and COST_OK
verdict cannot be carried forward -- they described the raw
`bankruptcyliquidation` population (106/yr), not a population that can
actually be identified and entered without lookahead. No population
recount, no re-run of `screen_design`, and no cost re-screen were
performed at scale, per the task's explicit instruction not to rescue a
hypothesis by guessing at the population a working discovery process
would need to produce.

**This closes the entire VIABLE side of the backlog for now.** Every
candidate that cleared the original breadth screen has since failed a
deeper, more specific check: merger arb on breadth-after-eligibility-
filtering, sub-$300M momentum on cost, and now distressed/post-reorg
equity on identity discoverability. The pattern across all three, and
across index deletion before them, is the same: the cheapest decisive
test is never the first number the screen produces -- it is whichever
requirement (a verifiable date, a tractable position, a real cost, a
discoverable identity) the study's OWN mechanism turns out to need most.

---

## 2026-09-06 — The first cost screen was wrong twice over: one bad estimator, one unit-confused turnover model, and correcting both flipped most of the backlog

Direct follow-up to the backlog-wide cost screen immediately below. Asked
to increase sample sizes, validate the spread estimator independently,
recheck whether the turnover assumption reflected real holding structure,
and screen the wider S&P 500+400+600 family -- three of those four checks
found the FIRST cost screen was measuring the wrong thing, not that the
candidates were genuinely cost-dead.

**Corwin-Schultz alone is not trustworthy, confirmed on real data.**
Validated against Abdi-Ranaldo (2017), an independently-derived
close/high/low estimator, on AAPL and MSFT -- names with a well-known real
spread of ~0.5-2bps. AAPL: Corwin-Schultz 18.5bps, Abdi-Ranaldo 0.0bps,
over the identical year of bars. Traced to the mechanism, not just
observed: Corwin-Schultz's top divergent days were genuine 4-5% intraday
trading ranges (real volatility, not spread), which its 2-day range
comparison partially misattributes as spread -- worse on higher-volatility
names, which is most of this backlog. One estimator agreeing with itself
was never evidence; a second, differently-derived estimator was needed and
now is reported for every candidate.

**The bigger bug: event arrival rate is not portfolio turnover.** The
first cost pass used each event-driven candidate's own events/year (built
for the MDA/breadth calculation) as if it were also the annual round-trip
count for cost purposes -- e.g. distressed equity's 106.21 events/year
became "106 round trips of one capital slot per year," an absurd 174%/yr
cost. The actual relationship (Little's-law argument): a strategy holding
N concurrent positions, each for T years, turns over 1/T times per year
PER DOLLAR OF CAPITAL, regardless of the event arrival rate -- N slots x
(1/T) turns/slot/year x (1/N) capital/slot = (1/T) turns/year of total
capital. Events/year and holding period T are independent quantities that
happened to get conflated because `screen_design`'s `positions=1,
rebalances=events/year` shape (built for statistical-independence/breadth
purposes) looks structurally identical to a turnover model, but answers a
different question. Refit with real, disclosed, per-candidate holding-
period assumptions (15 trading days for a short reversal trade, ~6 months
for merger arb's announcement-to-close, ~9 months for a slower distressed
re-rating) instead.

**Net effect: correcting both bugs reversed most of the backlog's cost
verdicts.** Index addition, spinoff dumping, and -- most importantly --
distressed/post-reorg equity all moved from apparently COST_DOMINATES to
COST_OK under both estimators. Distressed/post-reorg is now the strongest
surviving candidate in the whole backlog (2.82%/yr MDA, 0.25-1.66%/yr
cost) -- though it still carries its own unresolved data-shape caveat
(no confirmed link from a `bankruptcyliquidation` row to a continuing,
tradable post-reorg security). Sub-$5 stocks and sub-$300M momentum
remained COST_DOMINATES, and MORE robustly so than the first pass (larger
samples, both estimators agreeing) -- the correction did not rescue
everything, it fixed the measurement.

**S&P 500+400+600: re-confirmed data-blocked for 400/600 (two more
independent doc fetches plus a dozen guessed table names, all 403 -- a
genuine Sharadar product-scope gap, not an entitlement issue; the paid
tier works correctly on every table Sharadar actually sells), and the
available S&P-500-only version is the single most promising open thread
in the backlog: 4.16%/yr MDA (barely over the 4% ceiling) at essentially
ZERO measured cost (0.00%/yr Abdi-Ranaldo) -- large, liquid, index-eligible
names genuinely do avoid the cost problem that sank everything else.
Tripling the event population (if 400/600 data were ever sourced) would
plausibly clear the ceiling via the same sqrt(3) breadth-scaling argument
already used for the original index-deletion closure.

**The general lesson**: a "cost screen" is not just "estimate a spread and
multiply" -- both the estimator and the turnover model need their own
validation before the resulting number can be trusted, exactly the same
"cheapest decisive test on the ACTUAL requirements" discipline this
project has now applied to breadth (index deletion, merger arb), identity
(ticker reuse), and now cost twice in immediate succession.

---

## 2026-09-06 — Ticker identity is provably unsafe at micro-cap scale, and two more preregistration amendments caught problems that would have surfaced only after a backtest existed

Direct continuation of the sub-$300M momentum work below. Two things were
resolved before building any real ingestion pipeline.

**Ticker-symbol identity is not just "ambiguous" (the earlier free-tier
finding) -- it is confirmed unsafe, on two concrete real examples.**
Querying `FB` today returns a DIFFERENT, currently-active company
(permaticker `644713`, `isdelisted=N`) -- Sharadar's per-ticker lookup
returns whoever holds a ticker NOW, with no path back to a vacated
ticker's prior holder (Meta itself is permaticker `194817`, found only by
already knowing to look under `META`). Worse: `tickerchangefrom`/
`tickerchangeto` action rows are not reliably real-time-accurate either --
PNST (Pinstripes Holdings)'s recorded 2024-01-02 change to `BYN` never
took effect in the actual price feed; PNST kept trading under its own
ticker for over a year, until its real 2025-03-05 delisting, and `BYN` has
zero price rows, ever. Querying `stocks` by `permaticker` directly is not
supported (HTTP 400) -- price history can only be fetched by ticker
string, which means there is no clean way to stitch a security's full
history across a rename even when permaticker theoretically identifies it
correctly. **Fail-closed mitigation adopted**: every price fetch is
clamped to a ticker's own confirmed `firstpricedate`/`lastpricedate`
tenure; a security whose ticker changes mid-formation-window is excluded
from that month (missing data) rather than risking a silent splice of two
unrelated companies' price series. This matters far more here than it did
for the 29-name Dow roster this project has otherwise operated at --
micro-caps churn tickers constantly (reverse splits, SPAC suffix changes,
distress-driven renames), so this is routine exposure, not a corner case.

**Phase 1 of the PIT store build found a genuine, sharp coverage
boundary, not a bug.** Sharadar's `daily.marketcap` has exactly zero
coverage before 1998-12-01 (confirmed: 1998-11-30 returns 0 of ~7,220
names market-wide; 1998-12-01 returns full coverage) -- an 11-month
correction to the preregistered development window, immaterial on its
own. The raw percentile-based (bottom-quintile) eligible population WAS
confirmed stable across the full 1998-12 to 2026-09 history this time (a
full build, not a spot check) -- minimum 996 eligible names in any single
month, comfortably above the 30/50 preregistered portfolio sizes. This
resolves the "not measured" caveat the original backlog screen explicitly
left open.

**Two amendments made before Phase 2 touched anything, both decisive,
both reported rather than resolved unilaterally.** First, the existing
cost model (`engine/data.py`'s spread tiers, calibrated on Dow mega-caps,
bottoming out at "<$300M/day average dollar volume -> 5bps") is roughly
600-1000x too coarse for a $500K-$1M/day universe -- reusing it would
have silently modeled a $500K/day microcap as if it traded like a Fortune
500 company. Replaced with a real, standard technique (the Corwin-Schultz
2012 high-low spread estimator, which needs only daily bars, exactly why
it is the standard tool for exactly this problem) and three explicit cost
regimes (OPTIMISTIC/BASE/STRESS) plus a COST_SENSITIVE classification for
a result that survives BASE but not STRESS. Measured on 8 real names at
the liquidity floor: median round-trip spread 84.25bps, NOT monotonic
with ADV (a $974K/day name showed a higher spread than a $169K/day name,
because Corwin-Schultz responds to volatility as much as liquidity --
real signal, not noise). Sanity-checked against the preregistered top-30
portfolio's own 3.04%/yr MDA (25.33bps/month of detectable edge): at
realistic monthly turnover (>=50%, unmeasured but consistent with typical
monthly-rebalanced momentum), the OPTIMISTIC-regime (lower-bound,
spread-only) annualized cost ALREADY EXCEEDS the entire detectable
effect. Second, the liquidity floor's pass rate was checked against real
historical snapshots rather than assumed stable from today's number: 0%
in 2010 (0/25 sampled), 0% in 2015 (0/23), 12% in 2020 (3/25), vs. 28.6%
today -- a clear secular liquidity-growth trend, meaning the preregistered
development window (1998-12 through 2020-12) could not have supported
30/50-name portfolios under the $500,000/day floor for most of its span.

**Both findings were reported and the work paused, not resolved by
picking whichever design change happens to fix them.** A percentile-based
liquidity floor (the same nominal-vs-percentile fix already applied to
the market-cap threshold), a shorter study window, or a directly-measured
turnover rate might each address one or both findings -- but choosing
between them now, after computing that they help, is exactly the
after-the-fact parameter selection a preregistration exists to prevent.
Per this session's explicit instruction ("if modeled cost exceeds the
detectable effect, say so before running anything"), both findings are
recorded in `research/microcap_momentum_v1_preregistration.json`'s
`STATUS_amended` block and Phase 2 ingestion was not started. This is the
third candidate this project has walked right up to the edge of building
real infrastructure for, only to find the honest number didn't support it
-- after index deletion (breadth) and merger arb (breadth, again, after
an eligibility filter), this time the wall was cost and a nonstationary
liquidity floor rather than raw event count. The pattern across all three
is the same: run the cheapest decisive test on the ACTUAL requirements of
the study -- not the idealized version -- before building anything durable.

---

## 2026-09-06 — Merger arb's headline breadth number didn't survive its own eligibility requirement; sub-$300M momentum did, after the same scrutiny

Direct follow-up to the backlog-wide screen below. Before writing a
preregistration for the recommended merger-arb candidate, its two
load-bearing assumptions were checked rather than assumed: does a clean
announcement date exist, and does the original breadth number survive
requiring one.

**Announcement date: real, but not universal.** Sharadar's `events`
table (Material Corporate Events, 8-K-sourced) carries eventcode `11`,
which behaves like SEC Item 1.01 ("Entry into a Material Definitive
Agreement"). This was NOT trusted from the schema alone -- cross-validated
against real price action, confirming it cleanly on Poseida Therapeutics
(+228% same-day return exactly on its code-`11` filing date). But 27% of a
45-deal sample (2020-2025) had no code-`11` event at all (mostly foreign
private issuers, who file 6-K instead of 8-K -- a structural gap, not
noise), and stock-for-stock deals (confirmed via the separate
`acquisitioncash`/`acquisitionstock` action rows) show no announcement-day
price jump at all, mechanistically explained rather than a data failure --
a swap ratio doesn't re-rate the target the way cash consideration does.

**The eligibility filter needed for a trustworthy date also gutted the
breadth number, and that should have been expected.** The original screen
(below) reported 2.29%/yr MDA using every sub-$500M `acquisitionof` event
unconditionally -- 161/yr. Once restricted to what the study actually
needs (a verifiable announcement date confirmed by a real price jump, AND
a cash deal, since a stock deal needs an acquirer-side hedge this study
wasn't scoped to build), the eligible rate drops to ~50/yr, and MDA moves
to **4.10%/yr at the project's standard rho=0.5, 4.66%/yr at rho=0.2 (the
low-correlation case that specifically favors merger arb), 5.40%/yr at
rho=0 -- failing the 4%/yr ceiling under every assumption tried.** Widening
to <$1B deals recovers viability (3.71%/yr) but at that size large arb
funds routinely participate, which weakens the CAPACITY-barrier claim
rather than rescuing it -- a genuine breadth-vs-mechanism tradeoff, not a
fix, and widening a universe to rescue a failing screen is exactly what
this project's own guardrails prohibit.

**The general lesson, stated once so it doesn't need re-learning per
candidate**: a raw event count is not a usable-event count until every
eligibility requirement the STUDY ITSELF needs (a trustworthy entry
timestamp, a tractable position construction, a liquidity floor) has been
applied -- the same "measure the lowest-level record, don't trust the
aggregate" discipline this file has already learned twice (the prop
account P&L reconciliation; the index-deletion acquired/merged filter).
Every other event-driven candidate in the backlog screen should be
assumed optimistic until it receives the same treatment; distressed/
post-reorg equity in particular still carries its own unresolved version
of this exact problem (no confirmed link from a `bankruptcyliquidation`
row to a continuing, tradable post-reorg security).

**Sub-$300M cap momentum was tested with the same scrutiny and survived,
though its own headline number needed the same correction.** The original
screen's 0.91%/yr MDA modeled the strategy as holding its entire ~2,000
name eligible universe simultaneously -- not how a ranked cross-sectional
strategy is actually built. Recomputed against a realistic held-portfolio
size (top 20-75 names, matching this project's own Dual Momentum-style
concentration), MDA runs 2.41-3.38%/yr -- corrected downward from the
original number, same direction as merger arb's correction, but still
comfortably clearing the ceiling because the raw margin (0.91% vs. 4%,
>4x headroom) was large enough to absorb it. A liquidity/history floor
was measured directly (28.6% of a 70-name sample passes $500,000/day
median volume with 200+ trading days of history) rather than assumed, and
does not threaten that margin. Point-in-time integrity of
`daily.marketcap` -- the field this whole design depends on -- was
spot-checked against a real delisted security (SWY, delisted
2015-01-29) and found continuously accurate through its own actual last
trading day, not just for the S&P 500 mega-caps checked earlier this
session but for a name that matters to a micro-cap-specific design.

**Preregistered** (`research/microcap_momentum_v1_preregistration.json`)
before any return was computed: 12-1 month cross-sectional momentum, a
PERCENTILE-based universe (bottom quintile by market cap, not a fixed
nominal $300M threshold that would mean something different in 1998
dollars than 2026 dollars), a 4-configuration parameter family (portfolio
size 30/50 x liquidity floor $500K/$250K, fixed before any result is
seen), and a falsification criterion deliberately bound to the exact
factor-regression discipline that closed the original Dual Momentum
question below -- residual alpha after MKT/SMB/HML/UMD must clear a 95%
CI excluding zero and must not collapse once SMB alone is included, or
this is a size bet wearing a momentum label, not a distinct edge.

---

## 2026-09-06 — Backlog-wide breadth screen: "event-driven vs. cross-sectional" was the wrong axis; universe size and rebalance frequency are the real one

Follow-up to the index-deletion screen below, prompted by a direct question:
does that result generalize to every other candidate in
`research/hypothesis_backlog.md`? Rather than screen one hypothesis at a
time, all seven remaining candidates were screened in a single pass
(`engine/hypothesis_backlog_screen.py`, `reports/hypothesis_backlog_screen/`)
using the same 4%/yr MDA ceiling, before any of them got a preregistration
or a backtest.

**The proposed standing rule was "single-index-family event hypotheses are
structurally out of reach; cross-sectional hypotheses are where a question
can be resolved."** The screen broke this rule in both directions, and the
breaks are the actual finding:

- Small merger arb (below institutional minimum deal size) and distressed/
  post-reorg equity are both EVENT-DRIVEN and both cleared VIABLE
  (2.29%/yr and 2.82%/yr best-case MDA). What makes them work is not their
  shape -- it's that both draw events from Sharadar's MARKET-WIDE `actions`
  table (~5,500+ securities), not from a single ~500-name index family.
  161 and 106 usable events/yr respectively, vs. deletion's filtered 184
  over 28.5 YEARS (~6.5/yr).
- Tax-loss-selling reversal is CROSS-SECTIONAL and still screened out DEAD
  (8.15%/yr). It was cross-sectional but narrow (S&P-500-only, ~500 names),
  rebalanced only once a year, and its qualifying "losers" cluster hard on
  one macro factor -- measured directly on a 63-name/4-year sample: loser
  rate (Jan-to-Nov return < -20%) ranged from 46.0% in 2022 to 1.6% in 2025.
  82 nominal positions/yr behaved like far fewer independent bets.

**Corrected rule**: breadth is governed by universe size and observation
arrival frequency, not by the event-driven/cross-sectional label. A
single-index-family event is capped by that index's own reshuffle rate
(~6-25/yr for S&P 500 addition/deletion/spinoff, regardless of which side
of the reshuffle is screened -- confirmed on three separate S&P 500 event
types this session, not just deletion). A market-wide event population or
a broad, frequently-rebalanced cross-section both clear the ceiling
easily. The label was a reasonable first-pass heuristic (3 of 4 VIABLE
rows and the one DATA_BLOCKED row were consistent with it) but is not the
underlying variable, and `research/hypothesis_backlog.md` now says so
explicitly so the next candidate is triaged on the right axis.

**Two corrections to this session's own prior guesses, made explicit
rather than quietly fixed**, per this project's standing rule that a wrong
prediction gets corrected in place, not silently edited: the backlog
previously guessed index addition would fail "almost unchanged" from
deletion because their raw event counts are nearly identical (699 vs.
696). Wrong reason, right caution to re-screen: addition needs no
M&A-exclusion filter (an addition is, by definition, a security that
keeps trading), so its USABLE count is the full 699, not a filtered
subset -- moving it from where deletion landed (11.45%/yr) to a
meaningfully different, though still-MARGINAL, 5.87%/yr. Separately, the
backlog's spinoff-dumping estimate (73 events) was S&P-500-scoped when the
actual mechanism -- forced selling by holders whose mandate can't retain a
newly spun-off, often small-cap security -- applies market-wide;
Sharadar's market-wide `actions` table gives 570 spinoff events over the
same period (~20/yr), still MARGINAL but nearly 8x the originally-guessed
population.

**A second vendor pagination defect found while pulling this data**,
distinct from the free-tier entitlement issues in the entries below:
`SharadarClient.query_all`'s "fewer rows than page size means no more
data" assumption is false for the market-wide (no ticker filter) `actions`
endpoint specifically. A single request for `action=bankruptcyliquidation`
spanning 1998-2026 with `limit=10000` silently returned only 1,359 rows,
stopping dead at 2007-12-26 -- with 948 more real rows sitting unfetched
from 2008 onward, confirmed by re-querying that later window directly.
Every broad multi-decade pull in `engine/hypothesis_backlog_screen.py`
chunks by 4-year windows to route around this; the client itself was left
unmodified since fixing ingestion robustness is store-building work this
screening pass was explicitly scoped not to do, but it is now a
documented risk for whoever ingests this table for real.

**One clean, real discovery**: Sharadar's `acquisitionof`/`acquisitionby`
action rows carry the actual total deal value in $M directly (confirmed
against named 2025 examples, e.g. Roche's $925.9M acquisition of Poseida)
-- a market-wide, dated, sized M&A feed this repo did not previously know
it had access to, and the single best-quality data source found across
either screening pass this session.

Diagnostic only; no PIT store, no backtest, and no preregistration were
built for any of the eight candidates evaluated across the two screens.

---

## 2026-09-06 — Retired the surplus Dual Momentum forward shadows

The "Dual Momentum is momentum, not an edge" research line (below, closed
2026-08-11) had accumulated eight forward-tracking shadows beyond the
canonical paper-execution loop and its one dedicated forward test
(`engine/forward_tracking.py`, itself untouched by this cleanup): the
frozen 50/50 DM/MRM blend, the frozen vol-scaled blend, the regime ledger,
the hourly shadow, the plain-vs-residual-momentum shadow, two prop-scaling
shadows, and two self-funded shadows. All fed a question that had already
been answered. `api/main.py`'s hourly execution-scheduler loop and its
five-minute prop-snapshot loop no longer call any of their advance
functions; each module's docstring and ledger now carries a dated
`retired` marker, and no historical observation was deleted or modified.

**One module could not be cleanly split.** `engine/dm_mrm_forward.py`
tracks all four of its comparison legs (canonical DM, canonical MRM, the
fixed 50/50 blend, the vol-scaled blend) as one fixed set, explicit in its
own docstring ("fixed for the life of this experiment... never resized"),
appending all four values in a single NAV row per session. Retiring only
the two blend legs named for closure would have required a live schema
migration of `dm_mrm_forward_nav.json` while the ledger was in
production use -- judged too risky to do as a drive-by fix. The entire
four-leg comparison was retired instead, canonical DM/MRM columns
included. This does NOT touch canonical Dual Momentum's real paper
execution or its dedicated `forward_tracking.py` test, which are separate
infrastructure and keep running unchanged. If Market-Residual Momentum
needs its own standalone forward shadow going forward, that is a
deliberate follow-up to fork out of this ledger, not a decision made here.

Also fixed in the same pass: `test_canonical_dm_replacement_ledger_reconciles`
was failing because it reruns the canonical strategy against LIVE data and
pins an exact event count (81) that grows by one every real-world monthly
rebalance -- observed at 82 as of this session. Changed to a floor
(`>= 81`), which still catches a real regression (events disappearing)
without failing on ordinary calendar progress.

---

## 2026-09-05 — Index Deletion / Forced Selling screened out before any backtest was built: breadth, not data quality, is the wall again

Direct application of the power-curve breadth lesson from this file's
2026-08-XX Dual Momentum entry to a brand-new hypothesis, and a demonstration
that the "run the cheapest decisive test first" rule pays for itself: this
hypothesis was closed using ONE table already in cache
(`engine/index_deletion_screen.py`, `reports/index_deletion_screen/`)
**before** any part of the full PIT store (`research/PIT_DATA_REQUIREMENTS.md`
Phases 3-7 -- security identity, corporate-action reconstruction, PIT
fundamentals gating, delisting-return acceptance gate) was built.

**The hypothesis**: mechanical selling by benchmark-constrained index funds
around an S&P 500 deletion's effective date temporarily pushes the security
below fair value, producing a post-deletion reversal. Barrier: MANDATE
(a holder must sell regardless of price). Full writeup, including the
mandatory sub-period decay check and universe-scope decision, in
`research/hypothesis_backlog.md`.

**Two things had to be resolved before the screen even ran.** First,
Sharadar's own `sp500` table (discovered by reading their docs index, not
in the 4-table scope the earlier free-tier evaluation covered) turned out to
give 28.5 years of add/remove events WITH machine-classifiable removal
reasons in a `note` field -- 370 acquired/merged, 184 market-cap-change, 73
spinoff, 10 bankruptcy, 59 other. Second, and this mattered more than it
looked: **the acquired/merged bucket, the largest one, is not usable for
this hypothesis at all.** Checked empirically on three real 2015 examples
(SWY, LO, COV) -- `tickers.lastpricedate` equals the removal date in every
case, meaning the security stops trading entirely at deal close. There is no
post-deletion price series to test a reversal against for the majority of
raw "removed" rows. Only the 184 market-cap-change removals (a company
falling out of the index while remaining listed) are structurally capable of
producing the return series the hypothesis describes. **A raw event count is
not a usable-event count until you've confirmed the security has a
post-event return to measure at all** -- the same shape of "measure the
lowest-level record, don't trust the aggregate" discipline as the idle-cash
prop-account lesson elsewhere in this file, applied to event selection
instead of P&L reconciliation.

**The screen itself failed decisively, not marginally.** 184 usable events
clears the pre-registered 150-event floor. But minimum detectable alpha,
computed three independent ways through the existing
`engine/power_curve.py:screen_design` (already-reused infrastructure, zero
new statistical machinery written) came back at **11.45%/yr best case** --
essentially 3x the pre-registered 4%/yr ceiling -- and the modern-era
(2016-2026) sub-window was worse still (~17-20%/yr). Tripling the event
count by adding S&P 400/600 (checked and confirmed unavailable from
Sharadar: `sp400`/`sp600`/`midcap400`/`smallcap600`/`indices` all return
HTTP 403) would only buy back a sqrt(3) improvement, landing around 6.6%/yr
-- still short. This is not a "needs more data" problem at any tractable
scale; ~6.5 usable single-name events per year, even at full S&P 500 scope,
is too few independent bets for any economically plausible effect size to
clear significance in a realistic sample.

**A subtle bug was caught building the screen itself, worth its own
line.** `screen_design(..., avg_pairwise_corr=rho)` doesn't only haircut the
bet count of the design under test -- it also recomputes its own internal
Dual Momentum calibration anchor at that same `rho`, rather than holding the
anchor's own (empirically fixed) correlation constant. For a `positions=1`
design (one security per event, nothing to cross-correlate), varying `rho`
therefore changes ONLY the anchor's assumed bet count -- and because MDA
scales with `sqrt(anchor_bets)`, a HIGHER assumed correlation produced a
LOWER, better-looking MDA in this specific case, the opposite of what
"more correlation" should mean. First draft of this screen labeled a
zero-correlation variant "optimistic" and it was actually the worst of the
three tried. Caught by writing the variants down side by side and reading
the actual numbers rather than trusting the label -- the same "a label is a
claim, verify it against the number" discipline as the leaderboard-status
and Sharadar-verdict fixes elsewhere in this file. `screen_design` itself
was left unmodified (reused as-is per this session's scope), and the
surprising coupling is now a pinned regression test
(`tests/test_engine/test_index_deletion_screen.py`) rather than a comment
someone could invalidate by refactoring without noticing.

**Per this project's own Phase 8/11 rule, closing here was correct, not
premature.** No backtest was built, the threshold was not relaxed, and the
full PIT store was not implemented for this hypothesis -- it would have been
real infrastructure work in service of a question that cannot produce an
interpretable answer regardless of how good the data underneath it is.
`research/hypothesis_backlog.md` records the queue for what comes next and
explicitly warns that the sibling "index addition" hypothesis shares almost
the identical event count (699 additions vs. 696 removals over the same
period) and should not be assumed viable just because it shares a barrier
label -- it needs its own screen, not an inherited verdict.

---

## 2026-09-05 — Sharadar entitlement upgraded to Bundle/Full History mid-session; generalized reconstruction clears 4 of 5 flagged symbols and gives DOW a new explanation

Direct follow-up to the entry immediately below. Between that validation run
and this one, the user's Sharadar account moved from the free tier to the
paid Bundle (Full History) entitlement. **The stale report was not trusted
on the user's word alone** -- `engine.validate_sharadar_contract.detect_tier`
was re-run with `force_refresh=True` and empirically confirmed the new
entitlement (`full_history_or_broader`: WMT pre-2021 history and TSLA, a
genuinely non-Dow symbol, both now return rows) before anything else in this
session used it. Re-running the full validator now measures 29/29
`EQUITY_UNIVERSE` symbols instead of 27/29 -- INTC and DOW, previously
outside the free entitlement's current-Dow-roster boundary, are now
accessible, and DOW immediately surfaced as a NEW fifth discrepancy the free
tier could never have seen.

**The verdict function itself was reporting a stale premise.** `_verdict()`
only had two branches, both about whether to buy a still-free account
("YES buy a month" / "NO resolve free-tier failures first") -- so after the
real upgrade it kept printing "NO -- resolve free-tier failures before
paying" against an account that was already fully paid. Fixed with a third
branch keyed on `tier` no longer being free-limited: the decision now reads
"CONDITIONAL -- paid entitlement active, but N symbol(s) need a per-symbol
reconstruction" and stops implying a purchase decision that was already
made. A verdict string is itself a claim and needs the same "recompute, don't
trust what was true when it was written" discipline CLAUDE.md already
requires of `derive_status()` on the strategy leaderboard.

**`engine/reconstruct_hon_adjustment.py` generalized into
`engine/reconstruct_adjustment.py`** (symbol is now a parameter everywhere;
`run_panel()` reconstructs a list of symbols and flags any where the
reconstruction disagrees with vendor `closeadj` beyond 0.5% median --
the permanent detector CLAUDE.md's Sharadar section calls for). Running it
against all 5 flagged symbols (HON, NKE, HD, MCD, DOW) changed the picture
for 4 of them:

| symbol | reconstructed vs. vendor closeadj (median) | reconstructed vs. yfinance cache (median) | verdict |
|---|---:|---:|---|
| HON | 3.25% | 0.09% | vendor closeadj is wrong (unchanged from prior entry) |
| NKE | 0.013% | 1.05% | **vendor closeadj is right; yfinance cache is the outlier** |
| HD | 0.032% | 0.73% | **vendor closeadj is right; yfinance cache is the outlier** |
| MCD | 0.006% | 0.71% | **vendor closeadj is right; yfinance cache is the outlier** |
| DOW | 0.097% median / 1.15% max | 1.15% | one-event recency lag, see below |

**NKE/HD/MCD: the original "vintage difference, unresolved" guess from the
prior entry was wrong in direction.** All three reconstructions -- built
only from Sharadar's own `closeunadj` + `actions`, never referencing
yfinance -- match Sharadar's own `closeadj` to within a few basis points on
every single distribution event (worst residual across all three: 0.028%).
The ~0.7-1.06% divergence these three showed against the repo's cached
`data/{SYMBOL}_1d.parquet` is therefore evidence that the **yfinance-derived
local cache**, not Sharadar, carries the different adjustment vintage.
Lesson: "two adjusted series disagree" is not evidence about which one is
wrong, and guessing wrong here would have meant discarding the technically
correct vendor field in favor of the flawed one.

**DOW's discrepancy is a single-event recency artifact, not a systemic
one.** 20 of DOW's 21 real distribution events match the reconstruction to
within 0.01%; only the most recent dividend (ex-date 2026-08-31, five
calendar days before this session's 2026-09-05 fetch) shows `closeadj`
applying NO adjustment yet (`actualJump=1.0`) where the reconstruction
predicts a 1.16% jump. Reported as an as-of update-latency effect --
Sharadar's `closeadj` for a security appears not to incorporate its most
recent ex-dividend date immediately -- rather than folded into the same
bucket as HON's compound-event defect. Any PIT ingestion pulling `closeadj`
within days of a fresh ex-dividend date should expect this lag and prefer
the `closeunadj`+`actions` reconstruction for very recent history.

**Net effect on the PIT price-basis decision (Phase 3 below): use the
reconstruction as the canonical adjusted-close series, never vendor
`closeadj` directly, exactly as CLAUDE.md's Sharadar section already
specifies -- this session's evidence is why.** For 4 of 5 symbols tested,
`closeadj` was already correct and the reconstruction just confirms it
cheaply; for HON it caught a real, otherwise-invisible 3.25% defect in one
compound corporate action; for DOW it caught a transient adjustment lag a
naive `closeadj`-trusting ingestion would have baked into history as a
permanent ~1.15% price discontinuity at exactly the ingestion date. A
single reconciliation check earns its cost across the whole panel, not just
on the one symbol that happened to fail first.

---

## 2026-09-05 — Sharadar contract validation: free tier proves the wrong thing to justify a purchase decision yet

Follow-up to the 2026-08-31 "membership is tractable, prices are the wall"
entry: Sharadar (sharadar.com, `api.sharadar.com/v1.0`) was evaluated on its
free tier (Dow 30, 5 years) as a candidate paid (Bundle/Full History, $69/mo)
source for `engine/pit_all_stocks.py`'s PIT contract, using
`engine/sharadar_client.py` (isolated cache under
`data/sharadar_validation_cache/`, never touching `data/*.parquet`) and
`engine/validate_sharadar_contract.py` (reads required fields directly from
`engine.pit_all_stocks`'s constants — never a re-typed second list). Full
report: `reports/sharadar_contract_validation/report.md` /
`report.json`. **Verdict: NO — resolve free-tier failures before paying**
(measuredSymbols=27/29, discrepancies on NKE/HD/HON/MCD). Two of the report's
gates failed, and only one of them is something Full History would actually
fix.

1. **The free-tier price cross-check itself did not clear.** Comparing
   Sharadar's `stocks.closeadj` against the repo's existing
   `data/{SYMBOL}_1d.parquet` cache across the 29-name `EQUITY_UNIVERSE` (the
   `dow_pit` roster) over the ~5-year overlap found four symbols — NKE, HD,
   HON, MCD — above the 0.5%-median-divergence threshold (HON's median was
   3.24%, max 3.70%), while every other symbol cleared it. The *latest*
   common close agreed to within a few parts in a million on all four, which
   originally looked like the signature of two adjusted series computed at
   different as-of dividend/action vintages — an informed guess, not a
   resolved discrepancy, so per this project's own "name it, don't average
   it away" rule (CLAUDE.md) it was reported unresolved rather than waived.
   **Buying the paid tier would not itself resolve this** — Full History
   changes date coverage, not adjustment vintage.
   - **HON's case was then resolved, and the "vintage difference" guess was
     wrong.** `engine/reconstruct_hon_adjustment.py` independently
     recomputed HON's total-return close from `closeunadj` (real, as-traded,
     never split-adjusted) plus the `actions` table's own dated
     `split`/`dividend`/`spinoffdividend` rows, using a backward cascade
     never referencing yfinance. Result
     (`reports/sharadar_contract_validation/hon_reconstruction.json`): the
     from-scratch reconstruction matches the local yfinance cache to within
     rounding noise (0.087% median, 0.58% max) and disagrees with Sharadar's
     own `closeadj` by almost exactly the ~3.24% the original cross-check
     found. Per-event diagnostics narrow it further: 21 of HON's 22 real
     distribution events (18 ordinary dividends, the 2025-10-30 Solstice
     Advanced Materials spinoff) match a standard split-adjusted-basis
     formula to within a few basis points; only ONE event — the compound
     2026-06-29 action (a 0.5-ratio `split` coincident with the Honeywell
     Aerospace/HONA `spinoffdividend`, value=221.01) — is off, by 3.26%, and
     that single event's residual alone accounts for the whole aggregate
     divergence (post-event dates match exactly; every pre-event date
     carries almost exactly this one constant offset, not 22 small
     disagreements spread evenly across five years). This points at
     Sharadar's own `closeadj` computation for that one compound corporate
     action, not at a generic adjustment-vintage difference and not at an
     error in yfinance — though the exact internal formula Sharadar used
     was never confirmed from vendor documentation (their actions-table docs
     don't specify per-action-type `value` semantics; see
     `validate_sharadar_contract.py`'s doc fetch), so this is reported as
     strong evidence, not a proven vendor bug. **The reusable finding**:
     a dividend/spinoffdividend `value`'s percentage impact must be computed
     against the SPLIT-ADJUSTED price in force at that date (raw
     `closeunadj` divided by the cumulative split factor), never the raw
     price directly — using the raw price is off by roughly the full split
     ratio for every event dated before a later split, confirmed against
     all 22 of HON's real events. NKE/HD/MCD were not re-investigated this
     way and still carry only the original vintage-difference guess.
2. **The free tier's entitlement boundary is the *current* Dow roster, not a
   fixed list of 29/30 names.** Probed empirically (WMT recent history:
   present; TSLA, a genuinely non-Dow active stock: HTTP 403; WMT pre-2021:
   zero rows) rather than assumed from Sharadar's marketing copy. INTC and
   DOW — both in this repo's July-2021-snapshot `EQUITY_UNIVERSE` but no
   longer in the live Dow — returned zero rows under the free key, which is
   the correct free-tier behavior, not a bug, but it means a literal
   "free tier covers the Dow 30" claim already needs qualification before
   any purchase decision is made on top of it.
3. **A vendor's adjustment-method docs describing "delivered vs. imputed"
   fields must be verified against an actual in-window corporate action, not
   trusted from prose.** Sharadar's FAQ states methods 1 (unadjusted) and 3
   (split+dividend+spinoff-adjusted) deliver only `closeunadj`/`closeadj`
   directly and require O/H/L to be imputed by ratio against method 2's
   directly-delivered split-adjusted O/H/L. Verified empirically against
   WMT's real 2024-02-26 3-for-1 split: unadjusted close showed the ~3x
   discontinuity, split-adjusted and fully-adjusted closes stayed
   continuous across the event — and critically, a query with `to` set
   *before* the split date still returned split-adjusted prices already
   divided by three. **The vendor's `to` parameter is not an as-of
   adjustment boundary** — same shape of bug this project already hit with
   Yahoo's `Adj Close` in the 2026-08-31 entry above, confirmed again in a
   completely different vendor. Any future PIT ingestion from Sharadar must
   treat every adjusted price as reflecting the security's *entire*
   adjustment history through the fetch date, never through the requested
   window.
4. **A permanent-ID claim is not satisfied by the field existing.**
   `tickers.permaticker` is documented as a stable identifier, but the
   validator requires an accessible ticker-change action row whose old and
   new ticker both resolve to the same `permaticker` before calling identity
   continuity PRESENT rather than AMBIGUOUS — same "ticker continuity is not
   security continuity" principle as lesson 4 in the entry above. Zero such
   linked examples were found under the free entitlement's visible action
   history, so this stays unverified rather than assumed.
5. **An untestable check must be reported as untestable, never silently
   passed.** Pre-2021 history, non-Dow coverage, historical delisted-security
   population, and delisting returns are all structurally outside what a
   5-year current-Dow key can measure — `tier_capabilities()` reports these
   as `UNTESTABLE-ON-THIS-TIER` explicitly rather than omitting the row, and
   a dedicated test (`test_free_tier_incapable_checks_are_never_reported_as_passes`)
   pins that an incapable check can never read as `PRESENT`.

**Why the verdict is NO despite most fields mapping cleanly.** Two Dow names
(INTC, DOW — no longer current Dow constituents) returned zero rows purely
because of the free tier's current-roster boundary; Full History would
plausibly fix that half of the gap by itself. But the other half — the four
adjustment-vintage discrepancies — is a same-tier, same-window question that
buying broader date/symbol coverage does not touch; nothing about Full
History changes *which* dividend-adjustment vintage `closeadj` reflects for
a name already inside the free entitlement. Paying $69 now would leave
NKE/HD/HON/MCD exactly as unresolved as they are today. The correct next
step is resolving that divergence source on the free tier (or accepting it
as a known, disclosed adjustment-vintage difference with evidence, not
assumption) before the coverage-expansion questions become the bottleneck
worth paying for.
