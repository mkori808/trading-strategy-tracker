...
**RETIRED 2026-09-06.** Surplus forward shadows (both prop-scaling shadows
and both self-funded shadows, 0.20x/0.25x) from the Dual Momentum research
line, which closed 2026-08-11 (LESSONS.md: "Dual Momentum is momentum, not
an edge"). `api/main.py` no longer calls `collect_once` on its five-minute
schedule, so `prop_forward.db` stops receiving new snapshots here; its
historical observations were not deleted or modified. See
``logs/prop_forward.RETIRED.json`` for the machine-readable marker (a
sidecar file, not a schema change to `DB_PATH`).
"""
