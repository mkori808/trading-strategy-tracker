...
REQUIRED_PRICE_BASIS = "total_return_adjusted_ohlcv"
REQUIRED_MANIFEST_FIELDS = ("source", "snapshotId", "coverageStart", "coverageEnd")
...
# Not in REQUIRED_DAILY_COLUMNS: absent daily history stays runnable for
# strategies that never request a market-cap filter. See market_cap_available
# below and run_size_filtered_scan's minimum_market_cap gate.
...
def inspect_dataset(bundle_dir: Path = BUNDLE_DIR) -> PitDatasetStatus:
    ...
    if manifest.get("schemaVersion") != MANIFEST_SCHEMA_VERSION:
        invalid.append(f"manifest.schemaVersion must equal {MANIFEST_SCHEMA_VERSION}")
    for key in REQUIRED_MANIFEST_TRUE:
        if manifest.get(key) is not True:
            invalid.append(f"manifest.{key} must be true")
    if manifest.get("priceBasis") != REQUIRED_PRICE_BASIS:
        invalid.append(f"manifest.priceBasis must be {REQUIRED_PRICE_BASIS!r}")
