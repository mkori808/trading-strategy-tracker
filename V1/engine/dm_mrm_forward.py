...
**RETIRED 2026-09-06.** Two of this module's four fixed comparison legs
(the frozen 50/50 blend, the frozen vol-scaled blend) are surplus forward
shadows from the research line that closed 2026-08-11 (LESSONS.md: "Dual
Momentum is momentum, not an edge"). The comparison set is explicitly
"fixed for the life of this experiment" per this module's own design (see
above) and all four legs share one NAV row per session
(`NAV_SERIES_KEYS`) -- there is no safe way to stop advancing only the two
blend legs without a live schema migration of `dm_mrm_forward_nav.json`.
Rather than risk corrupting that ledger, `api/main.py` no longer calls
`advance_completed_sessions` at all, retiring the WHOLE four-leg
comparison, canonical DM/MRM legs included. This does NOT retire canonical
Dual Momentum's real paper execution (that is `engine/execution.py` plus
`engine/forward_tracking.py`'s dedicated frozen forward test, both
untouched) -- only this module's own re-derived DM/MRM comparison columns,
which existed here solely to sit alongside the two now-retired blends. If
Market-Residual Momentum needs its own standalone forward shadow kept
running going forward, that requires deliberately forking it out of this
fused ledger, not a silent decision made here. See
``logs/dm_mrm_forward_operations.json``'s top-level ``retired`` key.
"""
