...
momentum, not an edge"). `api/main.py` no longer calls `advance`, so this
ledger stops advancing here; its historical observations were not deleted.
See ``logs/dm_optimized_63d_hourly_shadow.json``'s top-level ``retired`` key.
"""
